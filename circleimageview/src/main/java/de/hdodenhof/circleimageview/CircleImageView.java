...
public class CircleImageView extends ImageView {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mBitmap != null) {
            ...
            if (mRebuildShader) {
                ...
                /*mBitmap 定义paint着色 第二个参数代表x方向绘制完一个bitmap之后 剩余空间的绘制规则 第三个决定的是Y轴方向的*/
    ...
    private Bitmap getBitmapFromDrawable(Drawable drawable) {
        ...
        Log.i("cccccccccc","111=="+drawable.getIntrinsicWidth()+"=="+drawable.getIntrinsicHeight());
    ...
    private void updateDimensions() {
        ...
        /*mDrawableRect 与 mBorderRect 的设置 宽高都是view宽高的较小值*/
        mDrawableRect.set(mBorderRect);
        if (!mBorderOverlay && mBorderWidth > 0) {
            /*todo 这里如果设置负数 那么绘制的圈的直径 可能就大于原图的直径 出现的效果 就是相交的效果*/
    ...
    private void updateShaderMatrix() {
        ...
        /*位图的宽高比 小于mDrawableRect 的宽高比 */
        /*w1 / h1 = w2 / h2*/
        if (bitmapWidth * mDrawableRect.height() > mDrawableRect.width() * bitmapHeight) {
            /*如果图片原有的宽高比 大于 绘制区域的宽高比 那么保证高显示 全部 宽度可能被裁切*/
            scale = mDrawableRect.height() / (float) bitmapHeight;
            dx = (mDrawableRect.width() - bitmapWidth * scale) * 0.5f;
        } else {
            /*如果图片的宽度 小于绘制区域的 宽高比 那么保证宽度全部绘制 垂直有可能裁切*/
